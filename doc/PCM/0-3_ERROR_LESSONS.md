...
### 2026-01-29 Phase 7.8 GitHub CI Jest 測試失敗教訓

### 問題現象
1. **GitHub Actions 報錯**：在執行 `RUN JEST TESTS` 時失敗，其中 `app/page.test.tsx` 出現多處斷言失敗。
2. **錯誤訊息**：`Unable to find an element with the text: /市場概覽/i` 或找不到預期的狀態徽章。

### 底層根本原因
1. **UI 文本偏移**：Phase 7 的 Rich Aesthetics 重構將原本的靜態文本（如「市場概覽」、「AI Worker」）更改為更具設計感的標籤（如「市場導航儀」、「AI Core」）。
2. **測試案例未同步**：Jest 測試腳本仍保留舊版的硬編碼字串斷言，導致在 DOM 渲染正確但內容變更的情況下失效。

### 解決方案
1. **全局文本校準**：對齊 `app/page.test.tsx` 與最新版本的 `app/page.tsx` 文本內容。
2. **組件隔離驗證**：確保狀態徽章（StatusBadge）的標籤與 ChartWrapper 的標題完全匹配。
