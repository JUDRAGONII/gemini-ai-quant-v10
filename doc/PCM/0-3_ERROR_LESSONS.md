...
### 2026-01-25 API 頻率限制與 IP 保護教訓
 
 ### 問題現象
 1. **頻繁限流**：在大規模美股回補時，即便配置了多組 API Key 與 1.5s 延遲，仍頻繁觸發 `429 Too Many Requests`。
 2. **輪詢失效**：觸發 429 後立即旋轉金鑰（Rotate Key）至新帳號，仍會立即再次報錯 429。
 
 ### 底層根本原因
 1. **IP 級別限流**：Tiingo (及多數 API) 除了對 API Token 進行限額外，也會對客戶端 IP 進行頻率監控。當單一 IP 短時間內發射大量請求時，會啟動短期封鎖，此時切換 Key 也無效。
 2. **延遲不足**：1.5s 延遲（約 40 req/min）過於接近免費版的核心限額 (50 req/min)，容錯率極低。
 
 ### 解決方案
 1. **物理延遲倍增**：將請求間隔強制調升至 **3.0s**（20 req/min），遠低於限額，確保長效穩定。
 2. **深度冷卻機制**：一旦識別出 429，立即暫停程序 **60s** 並執行金鑰旋轉，給予 IP 層級足夠的冷卻時間。
 
 ### 預防重複犯錯的 Checkbox
 - [ ] 免費版 API 的頻率設定應保留至少 50% 的安全緩衝（Safety Buffer）。
 - [ ] 429 處理邏輯不應只有重試，必須包含顯著的「冷卻間隔 (Cool-off Period)」。
 - [ ] 考慮 API Provider 是否具備 IP 綜合限流機制，而非僅針對 Token。
 
 ---
 
 ### 2026-01-25 數據庫連接池超時教訓
 
 ### 問題現象
 - **Error**: `Timed out acquiring connection from connection pool.` (PostgREST Error PGRST003)。
 
 ### 底層根本原因
 - **資源競爭**：在大規模併發寫入（如 Backfill Manager 快速執行 Upsert）或程序頻繁啟動/終止時，連接池可能因前次連接未釋放或達到上限而無法分配新連接。
 
 ### 解決方案
 1. 確保數據擷取器具備物理延遲（如前述 3.0s），間接降低對數據庫的壓測頻率。
 2. 遭遇此錯誤時，程序應執行短暫暫停並自動重試。
 
 ### 預防重複犯錯的 Checkbox
 - [ ] 高頻回補程序必須配對防護性延遲，避免打爆地端 Supabase/PostgreSQL 容器的連接池。
