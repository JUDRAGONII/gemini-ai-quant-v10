...
import { Search, BrainCircuit, LayoutGrid, Zap, Filter } from 'lucide-react';
            ...
            <div className="relative border-b border-white/5 bg-black/20 backdrop-blur-md px-8 py-10">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-1">
                        ...
                        <h1 className="text-4xl font-black text-white tracking-tight">AI 智力決策中心</h1>
                        <p className="text-slate-500 text-sm italic">透過多代理人辯證與滯後相關性，捕捉市場深層邏輯。</p>
                    </div>
                    ...
                    <form onSubmit={handleSearch} className="relative group">
                        ...
                        <input
                            ...
                            placeholder="搜尋標的 (如: 2330, NVDA)..."
                            className="bg-white/5 border border-white/10 rounded-2xl pl-12 pr-6 py-4 w-full md:w-[320px] text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-all font-bold placeholder:text-slate-600"
                        />
                    </form>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        ...
                        <div className="p-6 bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-white/5 rounded-2xl">
                            <h4 className="flex items-center gap-2 text-sm font-bold text-indigo-400 mb-4 uppercase tracking-widest">
                                <Zap className="w-4 h-4" /> 快速過濾器
                            </h4>
                            <div className="space-y-2">
                                {['美股領先相關性', '台幣匯率敏感度', '宏觀流動性對沖', 'AI 辯證歷史'].map((tag) => (
                                    <button key={tag} className="w-full text-left px-4 py-2 text-xs bg-white/5 hover:bg-white/10 rounded-lg transition-colors text-slate-400 font-bold border border-white/5">
                                        {tag}
                                    </button>
                                ))}
                            </div>
