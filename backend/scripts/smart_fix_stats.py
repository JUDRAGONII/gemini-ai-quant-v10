...
def smart_deploy_stats():
    ...
    # 1. 建立統計表 (NEW TABLE, 不會被鎖)
    print("Creating category_stats table...")
    ...
    # 2. 更新 RPC 為非阻塞模式
    print("Updating RPC to non-blocking stats-first mode...")
    sql_rpc = """
    ...
    # 3. 嘗試分段建立觸發器 (帶 timeout)
    print("Attempting to create triggers with timeout...")
    tables = ['exchange_rates', 'economic_calendar', 'macro_indicators', 'stock_factors', 'evolution_genes', 'daily_price']
    ...
    for table in tables:
        try:
            print(f"Setting up trigger for {table} (3s timeout)...")
            cur.execute("SET lock_timeout = '3s';")
            cur.execute(f"DROP TRIGGER IF EXISTS trg_stats_{table} ON {table};")
            cur.execute(f"CREATE TRIGGER trg_stats_{table} AFTER INSERT OR DELETE ON {table} FOR EACH ROW EXECUTE FUNCTION update_category_stats();")
            print(f"Trigger {table} OK.")
        except Exception as e:
            print(f"Skipping trigger for {table} due to lock. Will sync count instead. Error: {e}")
            conn.rollback() if not conn.autocommit else None
    
    # 4. 背景精準計數
    print("Starting background accurate sync...")
    categories = [
    ...
    print("Smart Deployment Finished.")
    cur.close()
    conn.close()
