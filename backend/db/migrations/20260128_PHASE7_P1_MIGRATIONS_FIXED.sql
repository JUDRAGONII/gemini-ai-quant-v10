-- ============================================================
-- Phase 7: 資料庫補全 Migration P1 修正版腳本 (FIXED)
-- 執行日期：2026-01-28
-- 功能：修正 RLS 策略重複與 intraday_candles 結構衝突
-- ============================================================

-- Step 1: 建立 portfolio_performance 投資組合績效表 (與 P0 部分重疊)
-- ============================================================
...
DROP POLICY IF EXISTS "Users can view own portfolio performance" ON public.portfolio_performance;
...
DROP POLICY IF EXISTS "Service role full access to performance" ON public.portfolio_performance;
...
DROP POLICY IF EXISTS "Public read institutional" ON public.stock_institutional;
CREATE POLICY "Public read institutional" ON public.stock_institutional FOR SELECT USING (true);
DROP POLICY IF EXISTS "Service role full access" ON public.stock_institutional;
...
DROP POLICY IF EXISTS "Public read margin" ON public.stock_margin;
CREATE POLICY "Public read margin" ON public.stock_margin FOR SELECT USING (true);
DROP POLICY IF EXISTS "Service role full access" ON public.stock_margin;
CREATE POLICY "Service role full access" ON public.stock_margin USING (auth.jwt()->>'role' = 'service_role');

-- Step 4: 處理 intraday_candles 分K行情表衝突
-- ============================================================
DO $$
BEGIN
    -- 如果存在舊版 ts 欄位，表示是舊結構
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'intraday_candles' AND column_name = 'ts') THEN
        -- 備份舊資料表
        ALTER TABLE public.intraday_candles RENAME TO intraday_candles_old;
        RAISE NOTICE '已備份舊版 intraday_candles 為 intraday_candles_old';
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Public read intraday" ON public.intraday_candles;
CREATE POLICY "Public read intraday" ON public.intraday_candles FOR SELECT USING (true);
DROP POLICY IF EXISTS "Service role full access" ON public.intraday_candles;
...
DROP POLICY IF EXISTS "Public read calendar" ON public.economic_calendar;
CREATE POLICY "Public read calendar" ON public.economic_calendar FOR SELECT USING (true);
DROP POLICY IF EXISTS "Service role full access" ON public.economic_calendar;
CREATE POLICY "Service role full access" ON public.economic_calendar USING (auth.jwt()->>'role' = 'service_role');

SELECT '✅ Phase 7 P1 Migration (FIXED) 執行成功' AS message;
